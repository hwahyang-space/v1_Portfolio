...
<head>
    ...
    <title>프로필 수정 | <?php echo $_SESSION['user_name']."님"; ?></title>
                ...
                <div class="col-lg-6 col-md-6 col-sm-6 col-xs-12">
                  <div class="breadcomb-wp">
                    ...
                    <div class="breadcomb-ctn">
                      <h2>프로필 수정</h2>
                      <p>포트폴리오 업로드 시, 보여질 프로필을 수정합니다</p>
                    </div>
                  </div>
                </div>
              </div>
                  ...
                  <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
                      <div class="form-element-list">
                          <div class="basic-tb-hd">
                              <h2><?php echo "<b>".$_SESSION['user_name']."</b> 님"; ?>의 정보</h2>
                              <!--<p>세부적인 내용은 <a href="../me/info.php">정보 수정</a>으로 이동하세요</p>-->
                          </div>
                          <form method="post" action="../../profile_update.php" enctype="multipart/form-data">
                              <div class="row">
                                  ...
                                  <div class="col-lg-6 col-md-6 col-sm-6 col-xs-12">
                                      <div class="form-group">
                                          <div class="nk-int-st">
                                              ID: <input type="text" class="form-control" placeholder="ID" value="<?php $asdf = $_SESSION['user_name']; $conn = mysqli_connect("localhost", "hwahyang_pf", "rrFevF#rdwN2mA-@Ah=W8xfBn5R%4m+K", "hwahyang_pf"); if(mysqli_connect_errno()) { echo '서버와의 연결이 유효하지 않습니다. 잠시 후 다시 시도하여 주세요.'; exit; } mysqli_query($conn, "set session character_set_connection=utf8;"); mysqli_query($conn, "set session character_set_results=utf8;"); mysqli_query($conn, "set session character_set_client=utf8;"); $select_query = "SELECT id FROM users WHERE `name`= '".$asdf."'"; $result_set = mysqli_query($conn, $select_query); $row = mysqli_fetch_array($result_set); echo $row['id']; ?>" name="ID" id="ID">
                                          </div>
                                          <div class="nk-int-st"></div>
                                          <div class="nk-int-st">
                                              닉네임: <input type="text" class="form-control" placeholder="닉네임" value="<?php echo $_SESSION['user_name']; ?>" name="nickname" id="nickname">
                                          </div>
                                          <div class="nk-int-st"></div>
                                          <div class="nk-int-st">
                                              이메일: <input type="text" class="form-control" placeholder="이메일 주소" value="<?php $asdf = $_SESSION['user_name']; $conn = mysqli_connect("localhost", "hwahyang_pf", "rrFevF#rdwN2mA-@Ah=W8xfBn5R%4m+K", "hwahyang_pf"); if(mysqli_connect_errno()) { echo '서버와의 연결이 유효하지 않습니다. 잠시 후 다시 시도하여 주세요.'; exit; } mysqli_query($conn, "set session character_set_connection=utf8;"); mysqli_query($conn, "set session character_set_results=utf8;"); mysqli_query($conn, "set session character_set_client=utf8;"); $select_query = "SELECT email FROM users WHERE `name`= '".$asdf."'"; $result_set = mysqli_query($conn, $select_query); $row = mysqli_fetch_array($result_set); echo $row['email']; ?>" name="email" id="email">
                                          </div>
                                          <div class="nk-int-st"></div>
                                          <div class="nk-int-st">
                                              비밀번호 변경: <input type="password" class="form-control" placeholder="비밀번호 변경시에만 입력하세요!!!" name="password" id="password">
                                          </div>
                                          <div class="nk-int-st"></div>
                                          <div class="nk-int-st">
                                              <button class="btn btn-success notika-btn-success" type="submit">수정</button>
                                          </div>
                                      </div>
                                  </div>
                              </div>
